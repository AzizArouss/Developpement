...
class UserModel
{
        ...
        $phone)
    {
        ...
        if(empty($user) == false)
        {
            throw new DomainException
            (
                "Il existe déjà un compte utilisateur avec cette adresse e-mail 2"
            );
        }
	...
	{
        // var_dump(['$password'=>$password, '$hashedPassword'=>$hashedPassword, 'crypt($password, $hashedPassword)'=>crypt($password, $hashedPassword)]);
        // exit();
        // Si le cryptage du mot de passe en clair avec le hashed password comme sel 
        // est le même que la version hachée alors renvoie true.
